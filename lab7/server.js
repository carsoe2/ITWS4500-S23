...
app.post("/db/addFamous", async function (req, response) {
    await client.connect();
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (count === 0) {
                ...
                response.writeHead(301, {
                    Location: `https://carsoe2.eastus.cloudapp.azure.com/node/`
                }).end();
            } else {
                console.log("Item not added");
            }
...
app.post("/db/addCelebrity", async function (req, response) {
    await client.connect();
    let db_connect = client.db("famousPeople").collection("lab6");
    var name = req.body.name;
...
app.post("/db/addHero", async function (req, response) {
    await client.connect();
    var name = req.body.name;
...
//Add from actor api
app.post("/db/addActor", async function (req, response) {
    await client.connect();
    var name = req.body.name;
...
app.put("/db/update/:id", async function (req, response) {
    await client.connect();
    let db_connect = client.db("famousPeople").collection("lab6");
